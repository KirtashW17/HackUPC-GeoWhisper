...
- **TTL:** input numérico con unidad (minutos / horas / días). Mínimo `1 minuto`, máximo `30 días`. Sugerencias rápidas como botones (15min / 1h / 1día / 1 semana) pero el usuario puede escribir cualquier valor dentro del rango.
- **`max_views`:** input numérico libre. Mínimo `1`, máximo `1000`. Sugerencias rápidas (1 / 5 / 25 / ∞-dentro-del-rango).
- Una nota expira por **lo que se cumpla primero**: tiempo o vistas.

Si más adelante el equipo quiere experimentar con notas "permanentes", se hace tras el MVP, y probablemente como un tipo distinto (`Landmark`?) — ver `future.md`.
...
8. Migración + modelo `Note` (`content:text`, `latitude:decimal`, `longitude:decimal`, `expires_at:datetime`, `max_views:integer`, `views_count:integer`, `user:references`, `visibility:integer` enum, `language:string` con índice)
9. Validaciones (presence, rangos lat/lng, content length 1..500, TTL ≤ 30 días, `max_views` 1..1000)
10. Scope `active` (no expirada y bajo `max_views`) — *con `# TODO: revaluar cuando el RecurringJob purgue expiradas; podría volverse redundante` en el código*
11. Método `view!` que incrementa `views_count` atómicamente y devuelve si sigue viva
12. Tests de modelo (validaciones, scope `active`, `view!`, expiración por tiempo y por views)
