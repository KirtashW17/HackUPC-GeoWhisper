...
- **Stack:** Rails 8 + Hotwire (Turbo + Stimulus), SolidQueue for background jobs, SQLite in dev/test (Postgres possible later), Leaflet + OpenStreetMap for the map view (no API key needed), browser Geolocation API for capturing coordinates (note: requires a secure context — works on `localhost`, needs HTTPS elsewhere).
