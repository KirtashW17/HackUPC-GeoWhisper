...
- **Document public API with YARD.** Every public method, class, and module gets a YARD docstring: a one-line summary, `@param` for each argument with its type and meaning, and `@return` with type and meaning. Add `@example` when the call site isn't obvious. Private helpers can skip YARD if the name and signature are self-explanatory. *Why:* the project will outlive the hackathon; future readers (humans and assistants) need a stable, structured contract to learn the codebase from. *How:* keep docstrings concise — one line of summary plus tags is enough. Don't restate the body. When you touch existing code that's missing YARD, add it as part of the change.
- **Code is commented in English.** All inline comments, YARD docstrings, commit messages bodies, and identifier names are written in English, regardless of the language used in user-facing strings (which go through i18n). User-facing chat with the maintainers may be in Spanish; the code itself is not.
